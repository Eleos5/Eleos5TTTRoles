...
SWEP.PrintName = "Role Change Device (T-J)"
SWEP.Author = "You"
...
local TJ_COOLDOWN = 15.0
local TjRawName = "t_j" -- if you want to change the raw name used to exclude T-J, update here
...
function SWEP:PrimaryAttack()
    ...
    -- Ensure only T-J can use it. We try to get the player's role raw name if available,
    -- otherwise check for a numeric constant that might match (best-effort).
    local rd = nil
    if ply.GetRoleData then
        rd = ply:GetRoleData()
    -- elseif ply.GetRole and ply.GetRole() then
    --     -- fallback: try to access ROLES by numeric constant
    --     local currentIndex = ply:GetRole()
    --     if ROLES and ROLES[currentIndex] then
    --         rd = ROLES[currentIndex]
    --     end
    end

    -- if not rd or rd.nameraw ~= TjRawName then
    --     ply:ChatPrint("Only the T-J may use this device.")
    --     return
    -- end
    ...
    -- Pick a role index (numeric constant) — excludes T-J
    local newRoleIndex = PickRandomAllowedRole(TjRawName)
    ...
    self.NextUse = CurTime() + TJ_COOLDOWN
