local ROLE = {}

-- REQUIRED NAMES
ROLE.nameraw = "t_j"
ROLE.name = "T-J"
ROLE.nameplural = "T-Js"
ROLE.nameext = "a T-J"
ROLE.nameshort = "TJ"

ROLE.desc = [[The T-J starts as an Innocent but can transform into a random custom role using their device.]]

ROLE.shortdesc = "Starts innocent, but can transform into another custom role."

-- T-J IS AN INNOCENT-TEAM ROLE
ROLE.team = TEAM_INNOCENT

-- NO SHOP (you give the device via loadout)
ROLE.shop = nil

-- GIVE THE DEVICE ON SPAWN
ROLE.loadout = {
    "weapon_ttt_rolechanger_tj"
}

-- NO CREDITS
ROLE.startingcredits = 0

-- Health unchanged, so leave nil
ROLE.startinghealth = nil
ROLE.maxhealth = nil

-- ACTIVE ROLE
ROLE.isactive = true  -- Role is actually used in-round

-- No special selection rules needed
ROLE.selectionpredicate = nil

-- T-J does NOT act like a jester (change to true if you want that!)
ROLE.shouldactlikejester = false

-- No language translations
ROLE.translations = {}

-- CONVARS (optional example)
ROLE.convars = {
    pct = 0.15,        -- 15% pick chance
    max = 1,           -- Only 1 T-J
    min = 2            -- Minimum 2 players needed
}

RegisterRole(ROLE)

if SERVER then
    AddCSLuaFile()
end
